...
void sensors_applet_notify_active_sensor(ActiveSensor *active_sensor, NotifType notif_type) {
    ...
    switch (notif_type) {
        ...
        case HIGH_ALARM:
            ...
            } else if (active_sensor->sensor_values[0] >= active_sensor->sensor_high_low_value && notif_type == HIGH_LOW_ALARM) {

                /* assume high alarm condition */
                relation = _("is very high");
                limit_type = _("upper limit");
                limit_value = active_sensor->sensor_high_low_value;
			} else if (active_sensor->sensor_values[0] >= active_sensor->sensor_high_med_value && notif_type == HIGH_MED_ALARM) {

                /* assume high alarm condition */
                relation = _("is very high");
                limit_type = _("upper limit");
                limit_value = active_sensor->sensor_high_med_value;
...
gboolean sensors_applet_add_sensor(SensorsApplet *sensors_applet,
                                   ...
                                   const gchar *high_low_alarm_command,
                                   const gchar *high_med_alarm_command,
    ...
    if (NULL == sensors_applet->sensors) {
        ...
        sensors_applet->sensors = gtk_tree_store_new(N_COLUMNS,
                                 ...
                                 G_TYPE_DOUBLE, /* high low type */
                                 G_TYPE_DOUBLE, /* high med type */
                                 G_TYPE_DOUBLE, /* high type */
                                 G_TYPE_BOOLEAN, /* alarm enable */
                                 G_TYPE_STRING, /* low alarm command */
                                 G_TYPE_STRING, /* high low alarm command */
                                 G_TYPE_STRING, /* high med alarm command */
    ...
    if (settings_id != NULL && settings_id[0] != '\0') {
        ...
        gtk_tree_store_set(sensors_applet->sensors,
                           ...
                           HIGH_LOW_VALUE_COLUMN, g_settings_get_double (settings, HIGH_LOW_VALUE),
                           HIGH_MED_VALUE_COLUMN, g_settings_get_double (settings, HIGH_MED_VALUE),
                           ...
                           HIGH_LOW_ALARM_COMMAND_COLUMN, g_settings_get_string (settings, HIGH_LOW_ALARM_COMMAND),
                           HIGH_MED_ALARM_COMMAND_COLUMN, g_settings_get_string (settings, HIGH_MED_ALARM_COMMAND),
...
void sensors_applet_all_alarms_off(SensorsApplet *sensors_applet, GtkTreePath *path) {
    ...
    sensors_applet_alarm_off(sensors_applet, path, HIGH_LOW_ALARM);
    sensors_applet_alarm_off(sensors_applet, path, HIGH_MED_ALARM);
