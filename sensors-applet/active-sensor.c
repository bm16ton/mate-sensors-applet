...
typedef enum {
    ...
    LOW_SENSOR_VALUE,
    NORMAL_SENSOR_VALUE,
    HIGH_LOW_SENSOR_VALUE,
    HIGH_MED_SENSOR_VALUE,
    HIGH_SENSOR_VALUE,
...
static gdouble sensor_value_range_normalised(gdouble value,
                                          gdouble low_value,
										  gdouble high_low_value,
										  gdouble high_med_value,
                                          gdouble high_value) {
...
static SensorValueRange sensor_value_range(gdouble sensor_value,
    ...
    gdouble range;
    range = sensor_value_range_normalised(sensor_value, low_value, high_low_value, high_med_value, high_value)*(gdouble)(VERY_HIGH_SENSOR_VALUE);
...
static gboolean active_sensor_execute_high_low_alarm(ActiveSensor *active_sensor) {
    return active_sensor_execute_alarm(active_sensor, HIGH_LOW_ALARM);
}

static gboolean active_sensor_execute_high_med_alarm(ActiveSensor *active_sensor) {
    return active_sensor_execute_alarm(active_sensor, HIGH_MED_ALARM);
}
    ...
    if (gtk_tree_model_get_iter(model, &iter, tree_path)) {
        ...
        if (active_sensor->alarm_timeout_id[notif_type] == -1) {
            ...
            switch (notif_type) {
                ...
                case HIGH_LOW_ALARM:
                    active_sensor->alarm_timeout_id[notif_type] = g_timeout_add_seconds(timeout,
                                                                                        (GSourceFunc)active_sensor_execute_high_low_alarm,
                                                                                        active_sensor);
                    break;

                case HIGH_MED_ALARM:
                    active_sensor->alarm_timeout_id[notif_type] = g_timeout_add_seconds(timeout,
                                                                                        (GSourceFunc)active_sensor_execute_high_med_alarm,
                                                                                        active_sensor);
                    break;
...
static void active_sensor_update_icon(ActiveSensor *active_sensor,
    ...
    value_range = sensor_value_range(active_sensor->sensor_values[0],
                                     ...
                                     active_sensor->sensor_high_low_value,
                                     active_sensor->sensor_high_med_value,
...
static void active_sensor_update_graph(ActiveSensor *as, cairo_t *cr) {
    ...
    for (i = 0; i < MIN(as->num_samples, width); i++) {
        ...
        line_height = sensor_value_range_normalised(as->sensor_values[i],
                                                    as->sensor_low_value,
                                                    as->sensor_high_low_value,
                                                    as->sensor_high_med_value,
...
void active_sensor_update(ActiveSensor *active_sensor,
    ...
    gdouble sensor_high_low_value;
    gdouble sensor_high_med_value;
    ...
    if (gtk_tree_model_get_iter(model, &iter, path)) {
        ...
        gtk_tree_model_get(GTK_TREE_MODEL(sensors_applet->sensors),
                   ...
                   HIGH_LOW_VALUE_COLUMN, &sensor_high_low_value,
                   HIGH_MED_VALUE_COLUMN, &sensor_high_med_value,
        ...
        if ((get_sensor_value = sensors_applet_plugins_get_sensor_value_func(sensors_applet, sensor_interface)) != NULL) {
            ...
            if (display_mode == DISPLAY_ICON ||
                ...
                if ((sensor_value_range(sensor_value, sensor_low_value, sensor_high_low_value, sensor_high_med_value, sensor_high_value) != sensor_value_range(active_sensor->sensor_values[0], active_sensor->sensor_low_value, active_sensor->sensor_high_low_value, active_sensor->sensor_high_med_value, active_sensor->sensor_high_value)) ||
                    ...
                    active_sensor->sensor_high_low_value = sensor_high_low_value;
                    active_sensor->sensor_high_med_value = sensor_high_med_value;
            ...
            if (sensor_alarm_enabled) {
                if (sensor_value >= sensor_high_low_value || sensor_value >= sensor_high_med_value || sensor_value >= sensor_high_value || sensor_value <= sensor_low_value) {
                    ...
                    if (sensor_value >= sensor_high_low_value) {
                        active_sensor_alarm_on(active_sensor, HIGH_LOW_ALARM);
                    }

                    if (sensor_value >= sensor_high_med_value) {
                        active_sensor_alarm_on(active_sensor, HIGH_MED_ALARM);
                    }
