...
typedef struct {
    ...
    GtkLabel *low_value_label, *high_low_value_label, *high_med_value_label, *high_value_label;
    GtkAlignment *low_value_spinbutton_aligner, *high_low_value_spinbutton_aligner, *high_med_value_spinbutton_aligner, *high_value_spinbutton_aligner;
    GtkAdjustment *low_value_adjust, *high_low_value_adjust, *high_med_value_adjust, *high_value_adjust;
    GtkSpinButton *low_value_spinbutton, *high_low_value_spinbutton, *high_med_value_spinbutton, *high_value_spinbutton;

    /* alarm widgets */
    GtkLabel *alarm_header;
    GtkLabel *low_alarm_command_label, *high_low_alarm_command_label, *high_med_alarm_command_label, *high_alarm_command_label, *alarm_timeout_label;
    ...
    GtkEntry *low_alarm_command_entry, *high_low_alarm_command_entry, *high_med_alarm_command_entry, *high_alarm_command_entry;
...
static void sensor_config_dialog_high_low_value_changed(GtkSpinButton *spinbutton, SensorConfigDialog *config_dialog) {
    GtkTreeModel *model;
    GtkTreePath *path;
    GtkTreeIter iter;
    gdouble value;
...
static void sensor_config_dialog_high_med_value_changed(GtkSpinButton *spinbutton, SensorConfigDialog *config_dialog) {
    GtkTreeModel *model;
    GtkTreePath *path;
    GtkTreeIter iter;
    gdouble value;
...
static void sensor_config_dialog_alarm_toggled(GtkToggleButton *button, SensorConfigDialog *config_dialog) {
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(config_dialog->high_low_alarm_command_label), value);
    gtk_widget_set_sensitive(GTK_WIDGET(config_dialog->high_low_alarm_command_entry), value);
    gtk_widget_set_sensitive(GTK_WIDGET(config_dialog->high_med_alarm_command_label), value);
    gtk_widget_set_sensitive(GTK_WIDGET(config_dialog->high_med_alarm_command_entry), value);
...
static void sensor_config_dialog_high_low_alarm_command_edited(GtkEntry *command_entry, SensorConfigDialog *config_dialog) {
    sensor_config_dialog_alarm_command_edited(command_entry,
                                              config_dialog,
                                              HIGH_LOW_ALARM);
}

static void sensor_config_dialog_high_med_alarm_command_edited(GtkEntry *command_entry, SensorConfigDialog *config_dialog) {
    sensor_config_dialog_alarm_command_edited(command_entry,
                                              config_dialog,
                                              HIGH_MED_ALARM);
}
...
void sensor_config_dialog_create(SensorsApplet *sensors_applet) {
    ...
    gdouble low_value, high_low_value, high_med_value, high_value, multiplier, offset;
    gboolean alarm_enable;
    gchar *low_alarm_command, *high_low_alarm_command, *high_med_alarm_command, *high_alarm_command;
    gint alarm_timeout;
    IconType icon_type;
    ...
    gtk_tree_model_get(model, &iter,
                       ...
                       HIGH_LOW_VALUE_COLUMN, &high_low_value,
                       HIGH_MED_VALUE_COLUMN, &high_med_value,
                       HIGH_VALUE_COLUMN, &high_value,
                       ALARM_ENABLE_COLUMN, &alarm_enable,
                       LOW_ALARM_COMMAND_COLUMN, &low_alarm_command,
                       HIGH_LOW_ALARM_COMMAND_COLUMN, &high_low_alarm_command,
                       HIGH_MED_ALARM_COMMAND_COLUMN, &high_med_alarm_command,
    ...
    config_dialog->high_low_value_adjust = g_object_new(GTK_TYPE_ADJUSTMENT,
                                                  "value", 0.0,
                                                  "lower", -100000.0,
                                                  "upper", 100000.0,
                                                  "step-increment", 1.0,
    ...
    config_dialog->high_low_value_spinbutton = g_object_new(GTK_TYPE_SPIN_BUTTON,
                                                        "adjustment", config_dialog->high_low_value_adjust,
                                                        "digits", VALUE_DECIMAL_PLACES,
                                                        "value", high_low_value,
                                                        "width-chars", SPINBUTTON_WIDTH_CHARS,
                                                        NULL);

    gtk_widget_set_halign (GTK_WIDGET(config_dialog->high_low_value_spinbutton), GTK_ALIGN_START);
    gtk_widget_set_valign (GTK_WIDGET(config_dialog->high_low_value_spinbutton), GTK_ALIGN_START);

    config_dialog->high_low_value_label = g_object_new(GTK_TYPE_LABEL,
                                                   "label", _("Sensor _high_low value"),
                                                   "mnemonic-widget", config_dialog->high_low_value_spinbutton,
                                                   "use-underline", TRUE,
                                                   "xalign", 0.0,
                                                   NULL);

    g_signal_connect(config_dialog->high_low_value_spinbutton, "value-changed", G_CALLBACK(sensor_config_dialog_high_low_value_changed), config_dialog);

    config_dialog->high_med_value_adjust = g_object_new(GTK_TYPE_ADJUSTMENT,
                                                  "value", 0.0,
                                                  "lower", -100000.0,
                                                  "upper", 100000.0,
                                                  "step-increment", 1.0,
    ...
    config_dialog->high_med_value_spinbutton = g_object_new(GTK_TYPE_SPIN_BUTTON,
                                                        "adjustment", config_dialog->high_med_value_adjust,
                                                        "digits", VALUE_DECIMAL_PLACES,
                                                        "value", high_med_value,
                                                        "width-chars", SPINBUTTON_WIDTH_CHARS,
                                                        NULL);

    gtk_widget_set_halign (GTK_WIDGET(config_dialog->high_med_value_spinbutton), GTK_ALIGN_START);
    gtk_widget_set_valign (GTK_WIDGET(config_dialog->high_med_value_spinbutton), GTK_ALIGN_START);

    config_dialog->high_med_value_label = g_object_new(GTK_TYPE_LABEL,
                                                   "label", _("Sensor _high_med value"),
                                                   "mnemonic-widget", config_dialog->high_value_spinbutton,
                                                   "use-underline", TRUE,
                                                   "xalign", 0.0,
                                                   NULL);

    g_signal_connect(config_dialog->high_med_value_spinbutton, "value-changed", G_CALLBACK(sensor_config_dialog_high_med_value_changed), config_dialog);
    ...
    config_dialog->high_low_alarm_command_entry = g_object_new(GTK_TYPE_ENTRY,
                                                          "text", high_low_alarm_command,
                                                          "width-chars", 25,
                                                          "sensitive", alarm_enable,
                                                          NULL);

    g_free(high_low_alarm_command);

    config_dialog->high_low_alarm_command_label = g_object_new(GTK_TYPE_LABEL,
                                                      "use-underline", TRUE,
                                                      "label", _("Hi_gh alarm command"),
                                                      "mnemonic-widget", config_dialog->high_low_alarm_command_entry,
                                                      "xalign", 0.0,
                                                      "sensitive", alarm_enable,
                                                      NULL);

    g_signal_connect(config_dialog->high_low_alarm_command_entry,
                     "changed",
                     G_CALLBACK(sensor_config_dialog_high_low_alarm_command_edited),
                     config_dialog);

    config_dialog->high_med_alarm_command_entry = g_object_new(GTK_TYPE_ENTRY,
                                                          "text", high_med_alarm_command,
                                                          "width-chars", 25,
                                                          "sensitive", alarm_enable,
                                                          NULL);

    g_free(high_med_alarm_command);

    config_dialog->high_med_alarm_command_label = g_object_new(GTK_TYPE_LABEL,
                                                      "use-underline", TRUE,
                                                      "label", _("Hi_gh alarm command"),
                                                      "mnemonic-widget", config_dialog->high_med_alarm_command_entry,
                                                      "xalign", 0.0,
                                                      "sensitive", alarm_enable,
                                                      NULL);

    g_signal_connect(config_dialog->high_med_alarm_command_entry,
                     "changed",
                     G_CALLBACK(sensor_config_dialog_high_med_alarm_command_edited),
                     config_dialog);
    ...
    gtk_size_group_add_widget(config_dialog->size_group, GTK_WIDGET(config_dialog->high_low_value_spinbutton));
    gtk_size_group_add_widget(config_dialog->size_group, GTK_WIDGET(config_dialog->high_med_value_spinbutton));
    ...
    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_low_value_label),
                    1, 5, 1, 1);

    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_low_value_spinbutton),
                    2, 5, 1, 1);

    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_med_value_label),
                    1, 6, 1, 1);

    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_med_value_spinbutton),
                    2, 6, 1, 1);
    ...
    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_low_alarm_command_label),
                    1, 12, 1, 1);

    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_low_alarm_command_entry),
                    2, 12, 1, 1);

    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_med_alarm_command_label),
                    1, 13, 1, 1);

    gtk_grid_attach(config_dialog->grid,
                    GTK_WIDGET(config_dialog->high_med_alarm_command_entry),
                    2, 13, 1, 1);
